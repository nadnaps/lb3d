...
void
FUNCTION(xdrfgetpos) ARGS(`xdrid, pos')
int *xdrid;
long long int  *pos;
{
unsigned long rocksize=0;
...
void FUNCTION(vtkopen) ARGS(`fid,
                            STRING_ARG(filename),
                            datatype, 
                            nx,ny,nz,
                            status')
...
int *status;
{
  ...
  *status = 0;
  ...
  fpvtk = fopen(STRING_PTR(filename),"a");
...
void FUNCTION(vtkfloat) ARGS(`fid, numfloats, floatvar, status')
int *fid;
int *numfloats;
float *floatvar;
int *status;
...
----------------------------------------------------------------------*/
void FUNCTION(vtkclose) ARGS(`fid, status')
int *fid;
int *status;
{
  FILE *fpvtk;
...
void 
FUNCTION(xdrfdarrayset) ARGS(`gx, gy, gz, nx, ny, nz, v_size')
...
FUNCTION(xdrfdarrayfloat) ARGS(`xdrid, fp, ret, x, y, z, nx, ny, nz, pos')
int *xdrid, *ret;
float *fp;
...
FUNCTION(xdrfdarraydouble) ARGS(`xdrid, fp, ret, x, y, z, nx, ny, nz, pos')
int *xdrid, *ret;
double *fp;
int *x, *y, *z, *nx, *ny, *nz, *pos;
{
