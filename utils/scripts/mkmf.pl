...
# the getrev step generates the file lb3d_revision.h, which sets the LBEREVISION constant to the svn revision number (if available).
print MAKEFILE "getrev:\n\techo \"#define LBEREVISION \\\"unknown\\\"\" | sed \"s/unknown/`svnversion`/\" | sed \"s/\\\"\\\"/\\\"unknown\\\"/\" > lb3d_revision.new";
